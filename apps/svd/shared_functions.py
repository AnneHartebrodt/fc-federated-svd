...
def svd_sub(data, ndims):
    ...
    s = np.flip(s)[0:ndims]
    v = np.flip(v.T, axis=1)[:, 0:ndims]
    u = np.flip(u, axis=1)[:, 0:ndims]
    return u,s,v, nd
...
def eigenvector_convergence_checker(current, previous, tolerance=0.000001, required=None):
    ...
    while col < current.shape[1] and not converged:
        ...
        #print('delte'+str(delta))
        deltas.append(delta)
        if delta >= 1 - tolerance:
            nr_converged = nr_converged + 1
        if nr_converged >= required:
            converged = True
        #print('nr_converged'+str(nr_converged))
        col = col + 1
    return converged, deltas

def convergence_checker_rayleigh(current, previous, alpha_current, alpha_prev, epsilon=1e-10,required=None):
    """
    ...
    if required is None:
        required = current.shape[1]
    while col < current.shape[1] and not converged:
        ra = np.dot(current[:,col].T, current[:,col])/alpha_current
        rap = np.dot(previous[:,col].T, previous[:,col])/alpha_prev
