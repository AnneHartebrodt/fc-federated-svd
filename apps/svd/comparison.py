...
import math as math
import apps.svd.convenience as cv
...
def compare_eigenvalues(federated, reference):
    diff = []
    for i in range(min(len(federated), len(reference))):
        diff.append(np.abs(federated[i]-reference[i]))
    return diff